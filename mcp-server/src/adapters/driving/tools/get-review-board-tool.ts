...
const InputSchema = {
  ...
  pin_name: z.string().optional().describe('Override the pin marker name pattern (regex). The default recognizes pin-marker names in English and Russian.'),
  comment_field_name: z.string().optional().describe('Override the comment-field name pattern (regex). The default recognizes comment- and note-like names in English and Russian.'),
  reference_name: z.string().optional().describe('Override the reference (mockup) frame name pattern (regex). By default the reference frame is detected structurally (the aligned non-screenshot column), not by name.'),
