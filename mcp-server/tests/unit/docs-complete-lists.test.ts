...
describe('Gate 5B: both pages that enumerate the blocking actions name all thirteen', () => {
  ...
  it('accounts for EVERY `action` key under src/, so no value shape can walk past', () => {
    ...
    expect(sites.length, 'the number of `action` keys under src/ changed').toBe(36) // 34 -> 36: the icon scan-cap branch in uncheckedToBlocking (raise_max_depth at depth<=4 / add_pairs_on_children above - both REUSED); earlier 32 -> 34: the module-hint fix_pair branch;
    expect(vocabularySites.length, `the number of \`action\` value sites in ${VERIFICATION_FILE} changed`).toBe(31); // 29 -> 31: the icon scan-cap branch; earlier 27 -> 29: the module-hint fix_pair branch
    ...
    expect(literals, 'the resolved literal count changed -- a conditional branch gained or lost').toBe(32); // 30 -> 32: the icon scan-cap branch; earlier 28 -> 30: the module-hint fix_pair branch
...
const MARKED_QUOTES: Record<string, number> = {
  'docs/agents/design-qa-skill.md': 16, // 15 -> 16: the budget-drop verdict quote (measured, resolves against the 'discrepancies found' literal in report.ts)
  'docs/coverage.md': 2,
};
/**
