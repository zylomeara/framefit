...
describe('code-connect ingest', () => {
  it('401 without a CI key', async () => {
    const res = await fetch(`${base}/api/code-connect/mappings`, { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ docs }) });
    expect(res.status).toBe(401);
  });
  it('403 for an unknown CI key', async () => {
    const res = await fetch(`${base}/api/code-connect/mappings`, { method: 'POST', headers: { 'content-type': 'application/json', 'x-ci-key': 'fmcp_ci_bad' }, body: JSON.stringify({ docs }) });
    expect(res.status).toBe(403);
  });
  it('stores normalized mappings for the key owner', async () => {
    const res = await fetch(`${base}/api/code-connect/mappings`, { method: 'POST', headers: { 'content-type': 'application/json', 'x-ci-key': 'fmcp_ci_good' }, body: JSON.stringify({ docs }) });
    expect(res.status).toBe(200);
    expect(calls.replace).toEqual({ userId: 'u1', count: 1 });
    expect((await res.json()).stored).toBe(1);
  });
  it('400 on malformed body', async () => {
    const res = await fetch(`${base}/api/code-connect/mappings`, { method: 'POST', headers: { 'content-type': 'application/json', 'x-ci-key': 'fmcp_ci_good' }, body: JSON.stringify({ nope: true }) });
