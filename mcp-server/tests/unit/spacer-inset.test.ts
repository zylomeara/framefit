...
// RECTANGLE spacer children ("Padding horizontal", 12w, full-height, flush to the edge; the
...
const spacer = (over: Partial<SpecChild> = {}): SpecChild =>
  ({ id: '5:2', name: 'Padding horizontal', type: 'RECTANGLE', rect: R(0, 0, 12, 120), ...over });
const kidA = (over: Partial<SpecChild> = {}): SpecChild =>
  ({ id: '5:3', name: 'square', type: 'FRAME', rect: R(12, 20, 80, 80), ...over });
const kidB = (over: Partial<SpecChild> = {}): SpecChild =>
  ({ id: '5:4', name: 'title', type: 'TEXT', textSnippet: 'Add new card', rect: R(112, 40, 200, 40), ...over });
...
const domKid = (x: number, w: number, over: Partial<DomChild> = {}): DomChild =>
  ({ kind: 'element', tag: 'div', rect: R(x, 20, w, 80), ...over });
const domText = (x: number, w: number): DomChild =>
  ({ kind: 'element', tag: 'span', rect: R(x, 40, w, 40), children: [{ kind: 'text', rect: R(x, 40, w, 40), text: 'Add new card' }] });
...
describe('the predicate: what converts and what must not', () => {
  ...
  it('an edge spacer folds: gap[0] is between REAL children and carries the true numbers', () => {
    const rows = diffPair(tile([spacer(), kidA(), kidB()]), snap([domKid(12, 80), domText(112, 200)]), opts);
    const g = row(rows, 'gap[0]');
    expect(g).toBeDefined();
    expect(g!.prop).not.toContain('Padding');            // the spacer is out of the label
    expect(g!.status).toBe('pass');                       // 20 vs 20 (112-92 both sides)
    const si = row(rows, 'spacer_inset');
    expect(si).toMatchObject({ status: 'pass' });
    expect(si!.note).toMatch(/leading 12px/);              // the folded extent is named
  ...
  it('a misspelled SECOND token still matches - the gate reads the first token only', () => {
    const rows = diffPair(tile([spacer({ name: 'Padding horisontal' }), kidA(), kidB()]),
      snap([domKid(12, 80), domText(112, 200)]), opts);
  ...
  it('an accent bar and a divider do NOT convert (name gate)', () => {
    for (const name of ['Accent bar', 'Divider']) {
      const rows = diffPair(tile([spacer({ name }), kidA(), kidB()]), snap([domKid(12, 80), domText(112, 200)]), opts);
  ...
  it('a truncated spacer-shaped node does NOT convert and its truncation warn survives', () => {
    const rows = diffPair(tile([spacer({ childrenTruncated: true }), kidA(), kidB()]),
      snap([domKid(0, 12, { rect: R(0, 0, 12, 120) }), domKid(12, 80), domText(112, 200)]), opts);
    expect(row(rows, 'spacer_inset')).toBeUndefined();
  ...
  it('a short (non-full-cross) edge rectangle does NOT convert (named ceiling)', () => {
    const rows = diffPair(tile([spacer({ rect: R(0, 52, 12, 12) }), kidA(), kidB()]),
      snap([domKid(12, 80), domText(112, 200)]), opts);
    expect(row(rows, 'spacer_inset')).toBeUndefined();
  });

  it('a spacer-only container does NOT convert (survivor guard) - rows byte-shape as today', () => {
    const rows = diffPair(tile([spacer()]), snap([domKid(12, 80)]), opts);
    expect(row(rows, 'spacer_inset')).toBeUndefined();
  });

  it('an INTERIOR spacer is not folded and is NAMED in the note (visible ceiling)', () => {
    const rows = diffPair(tile([spacer(), kidA(), spacer({ id: '5:9', name: 'Spacer mid', rect: R(92, 0, 20, 120) }), kidB()]),
      snap([domKid(12, 80), domText(112, 200)]), opts);
    const si = row(rows, 'spacer_inset');
  ...
  it('strict profile (tol=0): a sub-pixel-fraction spacer still converts (the clamp is load-bearing)', () => {
    ...
    const rows = diffPair(tile([spacer({ rect: R(0.4, 0.3, 12, 119.4) }), kidA({ rect: R(12.4, 20, 80, 80) }), kidB({ rect: R(112.4, 40, 200, 40) })]),
      snap([domKid(12, 80), domText(112, 200)]), { tolerancePx: 0 });
    expect(row(rows, 'spacer_inset')).toBeDefined();
  });

  it('a NON-flush spacer-named rectangle does NOT convert (flush gate)', () => {
    const rows = diffPair(tile([spacer({ rect: R(6, 0, 12, 120) }), kidA(), kidB()]),
      snap([domKid(12, 80), domText(112, 200)]), opts);
    expect(row(rows, 'spacer_inset')).toBeUndefined();
  });

  it('a near-full-main-extent rectangle does NOT convert (background sanity gate)', () => {
    const rows = diffPair(tile([spacer({ rect: R(0, 0, 359.5, 120) })]),
      snap([domKid(12, 80)]), opts);
    expect(row(rows, 'spacer_inset')).toBeUndefined();
  });
});

describe('the border-edge padding reading on folded edges', () => {
  it('DOM inset 12 -> padding-left 12 vs 12 pass; the numbers are border-edge', () => {
    const rows = diffPair(tile([spacer(), kidA(), kidB()]), snap([domKid(12, 80), domText(112, 200)]), opts);
    expect(row(rows, 'padding-left')).toMatchObject({ figma: 12, dom: 12, status: 'pass' });
  });

  it('DOM inset 20 -> FAIL delta 8 (the honest-red criterion)', () => {
    const rows = diffPair(tile([spacer(), kidA(), kidB()]), snap([domKid(20, 80), domText(120, 200)]), opts);
    const p = row(rows, 'padding-left');
    expect(p).toMatchObject({ figma: 12, dom: 20, status: 'fail' });
  });

  it('DOM has NO inset at all -> FAIL delta 12 (never 0v0)', () => {
    const rows = diffPair(tile([spacer(), kidA(), kidB()]), snap([domKid(0, 80), domText(100, 200)]), opts);
  ...
  it('fold 24 with DOM padding 12 and flush-at-12 content -> FAIL with srcChannel (fix_plan survives)', () => {
    const rows = diffPair(tile([spacer({ rect: R(0, 0, 24, 120) }), kidA({ rect: R(24, 20, 80, 80) }), kidB({ rect: R(124, 40, 200, 40) })],),
      snap([domKid(12, 80), domText(112, 200)], { paddings: { top: 0, right: 0, bottom: 0, left: 12 } }), opts);
    const p = row(rows, 'padding-left');
  ...
  it('a consecutive run of edge spacers sums into one inset', () => {
    const rows = diffPair(tile([
      spacer({ rect: R(0, 0, 12, 120) }),
      spacer({ id: '5:8', name: 'Padding extra', rect: R(12, 0, 6, 120) }),
      kidA({ rect: R(18, 20, 80, 80) }), kidB({ rect: R(118, 40, 200, 40) })]),
    snap([domKid(18, 80), domText(118, 200)]), opts);
    expect(row(rows, 'padding-left')).toMatchObject({ figma: 18, dom: 18, status: 'pass' });
    expect(row(rows, 'spacer_inset')).toBeDefined();
  });

  it('trailing demote-off lock: fold 12 + DOM declared padding-right 6 with content short of it -> FAIL survives', () => {
    // mirror of the leading demote-off test: dualDemote's border-edge re-addition would
    // demote this genuine trailing shortfall (fig 12 vs dom 6+6=12); disabled on the folded
    ...
    const rows = diffPair(tile([kidB({ rect: R(0, 40, 200, 40) }), kidA({ rect: R(268, 20, 80, 80) }), spacer({ id: '5:7', name: 'Padding end', rect: R(348, 0, 12, 120) })]),
      snap([domText(0, 200), domKid(274, 80)], { paddings: { top: 0, right: 6, bottom: 0, left: 0 } }), opts);
    const pr = row(rows, 'padding-right');
    expect(pr).toMatchObject({ figma: 12, dom: 6, status: 'fail' });
    expect(pr!.srcChannel).toBeDefined();
  });

  it('trailing DOM spacer box is removed symmetrically', () => {
    const rows = diffPair(tile([kidA({ rect: R(0, 20, 80, 80) }), kidB({ rect: R(148, 40, 200, 40) }), spacer({ id: '5:7', name: 'Padding end', rect: R(348, 0, 12, 120) })]),
      snap([domKid(0, 80), domText(148, 200), domKid(348, 12, { rect: R(348, 0, 12, 120) })]), opts);
    expect(row(rows, 'structure_mismatch')).toBeUndefined();
    expect(row(rows, 'padding-right')).toMatchObject({ figma: 12, dom: 12, status: 'pass' });
  });

  it('a trailing spacer folds symmetrically on padding-right', () => {
    const rows = diffPair(tile([kidA({ rect: R(0, 20, 80, 80) }), kidB({ rect: R(148, 40, 200, 40) }), spacer({ id: '5:7', name: 'Padding end', rect: R(348, 0, 12, 120) })]),
      snap([domKid(0, 80), domText(148, 200)]), opts);
    expect(row(rows, 'spacer_inset')).toBeDefined();
    const pr = row(rows, 'padding-right');
    expect(pr).toBeDefined();
    expect(pr!.figma).toBe(12);
  });
});
...
describe('the DOM-side spacer symmetric removal', () => {
  ...
  it('DOM spacer is 20 wide vs design 12 -> FAIL delta 8 through the border-edge reading', () => {
    const rows = diffPair(tile([spacer(), kidA(), kidB()]),
      snap([domKid(0, 20, { rect: R(0, 0, 20, 120) }), domKid(20, 80), domText(120, 200)]), opts);
    expect(row(rows, 'padding-left')).toMatchObject({ figma: 12, dom: 20, status: 'fail' });
  });

  it('fig spacer + single real child vs DOM [spacer, content]: no wrong unwrap of the content child', () => {
    const rows = diffPair(tile([spacer(), kidA({ rect: R(12, 20, 348, 80), children: [kidB({ rect: R(112, 40, 200, 40) })] })]),
      snap([domKid(0, 12, { rect: R(0, 0, 12, 120) }), domKid(12, 348, { rect: R(12, 20, 348, 80) })]), opts);
...
describe('path accounting and the trace invariant', () => {
  it('the live 2-child shape rides the unwrap road after conversion and stays honest', () => {
    const body: SpecChild = { id: '5:5', name: 'Body', type: 'FRAME', rect: R(12, 20, 348, 80),
      children: [kidA({ rect: R(12, 20, 80, 80) }), kidB({ rect: R(112, 40, 200, 40) })] };
    const rows = diffPair(tile([spacer(), body]), snap([domKid(12, 80), domText(112, 200)]), opts);
    expect(row(rows, 'spacer_inset')).toBeDefined();
  ...
  it('2v3 salvage transition: the trace DEMOTES to skip (padding rows suppressed) and the mismatch names the fold', () => {
    const rows = diffPair(tile([spacer(), kidA(), kidB()]),
      snap([domKid(12, 80), domText(112, 200), domKid(316, 40, { rect: R(316, 20, 40, 80) })]), opts);
    const si = row(rows, 'spacer_inset');
  ...
  it('the structure_mismatch TOTAL-skip road demotes the trace too (no padding row exists)', () => {
    ...
    const rows = diffPair(tile([spacer(), kidA(), kidB({ textSnippet: undefined })]),
      snap([domKid(12, 80), domKid(112, 80), domKid(212, 80)]), opts);
    const si = row(rows, 'spacer_inset');
  ...
  it('the layout_axis_mismatch road demotes the trace too', () => {
    ...
    const rows = diffPair(tile([spacer(), kidA(), kidB()]),
      snap([domText(112, 200), domKid(12, 80)]), opts);
  ...
  it('DOM removal is BOUNDED by the fig fold count: a rendered accent-bar box is NOT eaten', () => {
    ...
    const accent: SpecChild = { id: '5:11', name: 'Accent bar', type: 'RECTANGLE', rect: R(12, 0, 4, 120) };
    const rows = diffPair(tile([spacer(), accent, kidA({ rect: R(16, 20, 80, 80) }), kidB({ rect: R(116, 40, 200, 40) })]),
      snap([domKid(0, 12, { rect: R(0, 0, 12, 120) }), domKid(12, 4, { rect: R(12, 0, 4, 120) }), domKid(16, 80), domText(116, 200)]), opts);
    expect(row(rows, 'structure_mismatch')).toBeUndefined();
  ...
  it('figInsetOn reads the POST-conversion population: size demotes when raw boxes agree (acceptance 10)', () => {
    // fig 360 wide with a 12px spacer; DOM declares padding-left 12 - the content-box size.w
    ...
    const rows = diffPair(tile([spacer(), kidA(), kidB({ rect: R(160, 40, 200, 40) })]),
      snap([domKid(12, 80), domText(160, 200)], { paddings: { top: 0, right: 0, bottom: 0, left: 12 }, clientWidth: 360 }), opts);
  ...
  it('site 2: a spacer INSIDE an unwrapped wrapper converts - counts and padding both honest', () => {
    ...
    const wrapper: SpecChild = { id: '5:6', name: 'Body', type: 'FRAME', rect: R(0, 0, 360, 120),
      children: [spacer(), kidA(), kidB()] };
    const rows = diffPair(tile([wrapper]), snap([domKid(12, 80), domText(112, 200)]), opts);
    expect(row(rows, 'spacer_inset')).toBeDefined();
  ...
  it('dom-dom sides gate: the same fabricated spacer shape does not fold', () => {
    const rows = diffPair(tile([spacer(), kidA(), kidB()]), snap([domKid(12, 80), domText(112, 200)]),
