...
const resp: RawVariablesResponse = { meta: {
  ...
  variables: {
    'V1': { id: 'V1', name: '--ds-duration-fast', resolvedType: 'FLOAT', variableCollectionId: 'VC', valuesByMode: { m: 120 } } as any,
    'V2': { id: 'V2', name: '--ds-ease-standard', resolvedType: 'STRING', variableCollectionId: 'VC',
      valuesByMode: { m: { type: 'VARIABLE_ALIAS', id: 'VariableID:ABCDEF0123456789ABCDEF0123456789ABCDEF01/2:2' } } } as any,
  },
} } as any;

describe('--ds- variables are not dropped', () => {
  it('lists both --ds- tokens (literal + unresolved cross-library alias)', () => {
    const names = listTokens(resp).map((t) => t.name);
    expect(names).toContain('--ds-duration-fast');
    expect(names).toContain('--ds-ease-standard');
  });
