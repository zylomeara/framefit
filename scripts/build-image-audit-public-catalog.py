...
def _build_npm(
    ...
    for expected_id, reference in enumerate(references, 1):
        ...
        if len(decoded) != 64 or base64.b64encode(decoded).decode("ascii") != integrity[7:]:
            raise CatalogError("invalid npm integrity")
...
def build_catalog(
    *,
    ...
    budget = [0]
    npm, npm_artifacts = _build_npm(Path(npm_manifest), Path(npm_blobs), Path(npm_member_index), budget)
