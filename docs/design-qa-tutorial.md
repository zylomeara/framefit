...
- The receipt is budget-honest: if the blocking list is truncated, `blocking_capped` says how many
  ...
  `omitted_pair_ids` names them, a `verification.notes` line says whether any of them carried
  FAILing rows, and a dropped fail keeps the verdict red — re-run the named pairs in a smaller
  call to see their rows.
