...
`degraded_stages: [{ stage, reason, ms, detail }]` key, and one `ℹ️` line in `report_markdown`. Those
degraded `confirm_token` blockers are closable: their detail names the escalation road - run
`get_variables {timeout_ms: 120000}` on the file (the larger budget bypasses the capped fetch's
negative cache and warms it on success), then re-run the compare. `ms`
