...
(e.g. a button whose visible label lives in a node with a generic master name) — it also matches
...
| `focus_radius` | number 0.02–0.5 (default 0.12) | Focus-crop half-size as a fraction of node width (only used with `focus`). 0.12 ≈ a ~24%-wide window around the point. |
| `figma_token` | string | Override Figma PAT |
