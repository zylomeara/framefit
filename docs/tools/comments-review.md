...
| `fuzzy` | boolean (default `false`) | Typo-tolerant fuzzy matching. Default `false` uses fast exact substring. For word-form variants search by the common stem (e.g. `"updat"` matches update/updated/updating); enable fuzzy for typos. |
...
numbered pin markers, "comment field" frames of numbered notes, and reference ("Mockup") frames - the classic
...
Each note also resolves a `referenceNode` (the node under the pin in the lane's aligned reference/"Mockup"
...
| `pin_name` | string | Override the pin marker name pattern (regex). The default recognizes pin-marker names in English and Russian. |
| `comment_field_name` | string | Override the comment-field name pattern (regex). The default recognizes comment- and note-like names in English and Russian. |
| `reference_name` | string | Override the reference (mockup) frame name pattern (regex). By default the reference frame is detected structurally (the aligned non-screenshot column), not by name. |
...
| `focus_radius` | number 0.02–0.5 (default 0.12) | Focus-crop half-size as a fraction of the prod screenshot width. 0.12 gives a ~24%-wide window. |
...
| `reference_name` | string | Override the reference (mockup) frame name pattern (regex). |
| `figma_token` | string | Override Figma PAT |
