...
| Corner radius | one uniform px corner radius | the DOM side yields ONE comparable px number or it says so: all four CSS corners equal **and** a px length → diffed as that number (including the exponent form Chrome emits above `999999px`, e.g. `1e+06px`); **anything else the browser computed** → 👁 unchecked, never a pass (see limits). The corners are compared as strings rather than parsed values, because `parseFloat('8px 4px')` is 8 and would read an 8×4 ellipse as a circle. The one silence left is an empty computed value: nothing was computed, so no number and no row |
