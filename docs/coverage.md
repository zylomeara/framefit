...
| Corner radius | one uniform px corner radius | the DOM side yields ONE comparable px number or it says so: all four CSS corners equal **and** a bare px length → diffed as that number; anything else — corners that differ, a `%` radius, an elliptical `8px / 4px` — → 👁 unchecked, never a pass (see limits). The corners are compared as strings rather than parsed values, because `parseFloat('8px 4px')` is 8 and would read an 8×4 ellipse as a circle. Four equal corners carrying no number at all (a `calc()` with a percentage in it, which is the only form that survives computation unresolved) → no number and no row, as before |
...
| Radii Figma cannot express: per-corner, `%`, elliptical | Figma carries ONE px `cornerRadius`, so a per-corner Figma radius has no axis at all, and on the DOM side any radius that is not a single px number has nothing to be compared against: four differing corners, `border-radius: 50%`, or an elliptical `8px / 4px`. All three produce a 👁 unchecked `corner-radius` row with a `resolve_skip` blocking item — never a pass, and never a fail either, because the diff cannot say which corner or which axis is off. Verify by eye. (`50%` shipped its bare number until this was measured: on a 300×20 box the real corners are 150px and 10px, and it passed a Figma `cornerRadius` of 50.) |
