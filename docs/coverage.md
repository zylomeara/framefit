...
| Corner radius | uniform corner radius | the four CSS corners are compared as STRINGS, not as parsed numbers (`border-radius: 8px / 4px 40px` computes to four `"8px 4px"`-style pairs that `parseFloat` all reads as 8): all four equal and numeric → diffed as one number; any difference → 👁 unchecked, never a pass; all four equal but unreadable (`calc()`, an unset longhand) → no number and no row, as before (see limits) |
