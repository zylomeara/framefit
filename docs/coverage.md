...
| How much DOM one capture may carry | the extractor's own node budget, and it binds on every deployment: 90 nodes at `max_depth` 1–4, 180 at 5–8, under a static 300-node schema backstop that rejects a snapshot outright. Past the budget a subtree comes back `childrenTruncated` — flagged, never silently dropped — and the fix is a narrower pair, not a bigger request. On stdio this is the ONLY cap on a capture; the upload caps below never enter that path |
