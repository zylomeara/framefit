...
| Corner radius | uniform corner radius | per-corner asymmetric radii are not individually diffed (see limits) |
...
| DOM colors the parser can't read (`oklch()`, `color()`, transparent) | ℹ️ info "color not parsed — verify by eye" (fills, borders, shadows, gradient stops alike) |
...
| Viewport must match the frame width | otherwise geometry rows are 👁 unchecked via the viewport guard (asymmetric: a WIDER window with a centered layout is fine; narrower guarantees reflow); Chrome won't shrink its window below ~500px — use CDP emulation for narrow frames |
| Snapshot upload: 2 MB per POST | split large captures across several POSTs to the same `upload_url` (the limit is per POST, not per session; each POST returns its own `snapshot_ref` with indices restarting from 0); refs live 30 minutes (sliding) |
| Per-corner asymmetric radii | only the uniform corner radius is diffed; a node with four different corners has no radius axis in the diff — verify by eye |
